...
base <- base %>% 
  mutate(sex = factor(case_when(sex == 2 ~ "Varón",
         ...
         local = factor(case_when(local == 1 ~ "CABA",
                                  ...
                                  TRUE ~ NA_character_)),
         local2 = factor(case_when(local %in% c(1, 2) ~ "GBA",
                                   local == 3 ~ "Córdoba",
                                  local == 4 ~ "Mar del Plata",
                                  TRUE ~ NA_character_)),
...
demograficos <- base %>% 
  select(sex, grupo_edad, sitcony, hij, obs, pais, local, nivel_ed_2) %>% 
...
demograficos %>%
  tbl_summary(include = c(sex, grupo_edad, sitcony, hij, obs, pais, local, nivel_ed_2),
                 label = c(grupo_edad ~ "Edad", 
                           sex ~ "Sexo", 
                           sitcony ~ "Situación conyugal",
                           hij ~ "Cantidad de hijos",
                           obs ~ "Sistema de salud",
                           pais ~ "Lugar de nacimiento",
                           local2 ~ "Lugar de residencia",
...
base %>%
  ...
  geom_bar(stat = "identity", alpha = 0.7) +
  geom_text(aes(label = scales::percent(n, accuracy = 0.1, suffix = ""), y = n - 0.02), size = 3.5) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
...
ggsave("salidas/rama_actividad2.jpg", width = 8, height = 5, dpi = 300)
...
ggsave("salidas/habilidades_lectura.png", width = 8, height = 5)
...
ggsave("salidas/habilidades_numericas.png", width = 8, height = 5)
```
...
ggsave("salidas/ACM.png", width = 8, height = 5)
...
ggsave("salidas/ACM_suplementarias.png", width = 8, height = 5)
```
...
ggsave("salidas/sexo.png", width = 8, height = 5)
...
ggsave("salidas/edad.png", width = 8, height = 5)
...
ggsave("salidas/nivel_educativo.png", width = 8, height = 5)
...
ggsave("salidas/ramact.png", width = 8, height = 5)
...
ggsave("salidas/tamano2.png", width = 8, height = 5)
...
ggsave("salidas/ocupaciones_sd.png", width = 8, height = 5)
```
