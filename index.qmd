...
pacman::p_load(tidyverse, haven, summarytools, janitor, ggthemr)
...
base <- base %>% 
  mutate(across(frecuencia1:frecuencia14, ~ na_if(., -999))) 

base <- base %>% 
  mutate(across(frecuencia1:frecuencia14, ~factor(., levels = c(1, 2, 3, 4, 5),
                                                  labels = c("Nunca",
...
base %>% 
  ...
  ggplot(aes(x = tarea, y = desc(porcentaje), fill = categoria)) +
